...
struct Eagle3Impl : Model {
    ...
    Eagle3Impl(
        ...
        kv_caches = new KVCacheManager<T>(1, this->model->num_key_value_heads, this->model->head_dim);
        fc1 = new Linear<T>(this->model->hidden_size, this->model->hidden_size);
        fc2 = new Linear<T>(this->model->hidden_size, this->model->hidden_size);
        fc3 = new Linear<T>(this->model->hidden_size, this->model->hidden_size);
        mid_layer = new LayerEmbed<T>(this->model->hidden_size, this->model->intermediate_size, this->model->num_attention_heads, this->model->num_key_value_heads, this->model->head_dim, this->model->rms_norm_eps);
        this->norm = new RMSNorm<T>(this->model->hidden_size, this->model->rms_norm_eps);
        this->draft_lm_head = new Linear<T>(this->model->hidden_size, draft_vocab_size);
    ...
    void load_to_storage(std::string name, void* ptr) {
        if (name.substr(0, 6) == "eagle3") {
            if (name.substr(0, 10) == "eagle3.d2t"){
    ...
    void eagle_prefill(int num_history_tokens) {
        ...
        elementwise_add3(calc_stream, num_prev, this->model->hidden_size, this->fc1->output, this->fc2->output, this->fc3->output, this->fc3->output);
        ...
        elementwise_add(calc_stream, num_prev, this->model->hidden_size, this->fc3->output, layer_output, this->fc3->output);
    }
    ...
    void draft(int32_t* tree_draft_ids, int32_t* tree_position_ids, int32_t* cache_length, uint64_t* tree_attn_mask, int32_t* tree_parent) {
        ...
        { // d = 0
            this->norm->prefill(calc_stream, 1, this->fc3->output + (num_prev - 1) * this->model->hidden_size, nullptr);
            ...
            repeat(calc_stream, topk_per_iter, this->model->hidden_size, num_prev-1, this->fc3->output, this->fc1->output);
        ...
        for (int d = 1; d < this->num_iter; ++d) {
            ...
            cudaMemcpy(this->fc3->output, this->fc1->output, topk_per_iter * this->model->hidden_size * sizeof(T), cudaMemcpyDeviceToDevice);

            T* layer_output = nullptr;
            this->mid_layer->decode(topk_per_iter, this->eagle_padded_length, this->model->embedding->output, this->fc3->output, layer_output, this->eagle_position_ids, this->eagle_cache_length, Mask(eagle_mask_2d, topk_per_iter, topk_per_iter * d), this->kv_caches->caches[0]);
            layer_output = this->mid_layer->output;
            ...
            remap_hidden(calc_stream, topk_per_iter, this->model->hidden_size, this->topk_func_2->topk_pos, this->fc3->output, this->fc1->output, topk_per_iter);
