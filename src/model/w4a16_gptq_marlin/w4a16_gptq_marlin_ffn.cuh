...
struct W4A16GPTQMarlinGatedFFN {
    ...
    Norm<T> *ffn_norm;
    W4A16GPTQMarlinLinear<T> *gate_proj, *up_proj;
    ...
    W4A16GPTQMarlinGatedFFN(int hidden_size, int intermediate_size, float rms_norm_eps) {
        ...
        this->gate_proj = new W4A16GPTQMarlinLinear<T>(hidden_size, intermediate_size);
        this->up_proj = new W4A16GPTQMarlinLinear<T>(hidden_size, intermediate_size);
    ...
    void prefill(const Stream& stream, int32_t num_tokens, T* input, T* prev_output, T* a_tmp, float* c_tmp) {
        ...
        this->gate_proj->prefill(stream, num_tokens, this->ffn_norm->output, a_tmp, c_tmp);
        this->up_proj->prefill(stream, num_tokens, this->ffn_norm->output, a_tmp, c_tmp);
        gated_silu<T>(stream, num_tokens, this->intermediate_size, this->gate_proj->output, this->up_proj->output);

        this->down_proj->prefill(stream, num_tokens, this->up_proj->output, a_tmp, c_tmp);
    }
