...
struct W4A8QQQFFN {
    ...
    W4A8QQQFFN(int hidden_size, int intermediate_size, float rms_norm_eps) {
        ...
        this->gate_up_proj = new W4A8QQQLinear<T>(hidden_size, intermediate_size*2);
    ...
    int64_t init_output_ptr(Memory* memory, int32_t num_tokens, int64_t offset) {
        ...
        int64_t up_proj_end = this->gate_up_proj->init_output_ptr(memory, num_tokens, gate_proj_quant_end);
    ...
    void prefill(const Stream& stream, int32_t num_tokens, T* input, T* prev_output) {
        ...
        this->gate_up_proj->prefill(stream, num_tokens, this->gate_up_quantizer->output, this->gate_up_quantizer->output_scale);
