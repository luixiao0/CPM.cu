...
struct W4A8QQQLayer {
    ...
    W4A8QQQLayer(int hidden_size, int intermediate_size, int num_attention_heads, int num_key_value_heads, int head_dim, float rms_norm_eps) {
        ...
        this->attn = new W4A8QQQAttention<T>(hidden_size, num_attention_heads, num_key_value_heads, head_dim, rms_norm_eps);
        this->ffn = new W4A8QQQFFN<T>(hidden_size, intermediate_size, rms_norm_eps);
    }
