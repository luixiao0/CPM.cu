...
struct W4A16MarlinAttention {
    ...
    Norm<T> *attn_norm;
    W4A16Linear<T> *q_proj, *k_proj, *v_proj;
    W4A16Linear<T> *o_proj;
    T* output;
    ...
    W4A16MarlinAttention(int hidden_size, int num_attention_heads, int num_key_value_heads, int head_dim, float rms_norm_eps, int group_size, int bits, bool use_marlin) {
        ...
        this->q_proj = W4A16LinearFactory<T>::CreateLinear(hidden_size, num_attention_heads * head_dim, group_size, bits, use_marlin);
        this->k_proj = W4A16LinearFactory<T>::CreateLinear(hidden_size, num_key_value_heads * head_dim, group_size, bits, use_marlin);
        this->v_proj = W4A16LinearFactory<T>::CreateLinear(hidden_size, num_key_value_heads * head_dim, group_size, bits, use_marlin);
        this->o_proj = W4A16LinearFactory<T>::CreateLinear(hidden_size, num_attention_heads * head_dim, group_size, bits, use_marlin);
    }
