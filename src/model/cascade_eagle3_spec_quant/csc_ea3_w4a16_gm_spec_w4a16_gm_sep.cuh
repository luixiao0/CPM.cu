...
template <typename T>
struct CascadeEagle3W4A16GMSpecW4A16GMImplSep: Model {
    ...
    int forward_times;
     ...
     CascadeEagle3W4A16GMSpecW4A16GMImplSep(
        W4A16GPTQMarlinModelImpl<T>* model,
    ...
    void draft_decode(int32_t num_tokens, int32_t padded_length, int32_t* input, int32_t* position_ids, int32_t* cache_length, uint64_t* mask_2d, void* output) {
        ...
        this->draft_model->decode_eagle3_states(num_tokens, padded_length, input, position_ids, cache_length, mask_2d, output, this->ea_decode_low_state, this->ea_decode_mid_state, this->ea_decode_high_state);
    }
