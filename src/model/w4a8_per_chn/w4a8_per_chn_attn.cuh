...
#include "w4a8_per_chn_linear.cuh"

template <typename T>
struct W4A8PerChnAttention {
    int hidden_size;
    ...
    W4A8PerChnLinear<T> *q_proj, *k_proj, *v_proj;
    QuantizerFuseSum<T, true> *o_quant_invoker;
    W4A8PerChnLinear<T> *o_proj;

    T* attn_output;
    float *softmax_lse, *softmax_lse_accum, *oaccum;

    W4A8PerChnAttention(int hidden_size, int num_attention_heads, int num_key_value_heads, int head_dim, float rms_norm_eps) {
        ...
        this->q_proj = new W4A8PerChnLinear<T>(hidden_size, num_attention_heads * head_dim);
        this->k_proj = new W4A8PerChnLinear<T>(hidden_size, num_key_value_heads * head_dim);
        this->v_proj = new W4A8PerChnLinear<T>(hidden_size, num_key_value_heads * head_dim);
        this->o_quant_invoker = new QuantizerFuseSum<T, true>(hidden_size);
        this->o_proj = new W4A8PerChnLinear<T>(hidden_size, num_attention_heads * head_dim);
    }
