...
template <typename T>
struct W4A8PerChnGatedFFN{
    int hidden_size;
    int intermediate_size;
    float rms_norm_eps;

    RMSNormQuantFuseSum<T, true> *ffn_norm;
    W4A8PerChnLinear<T> *gate_proj, *up_proj;
    QuantizerFuseSum<T, true> *down_quant_invoker;
    W4A8PerChnLinear<T> *down_proj;
    ...
    W4A8PerChnGatedFFN(
        ...
        this->gate_proj = new W4A8PerChnLinear<T>(hidden_size, intermediate_size);
        this->up_proj = new W4A8PerChnLinear<T>(hidden_size, intermediate_size);
        this->down_quant_invoker = new QuantizerFuseSum<T, true>(intermediate_size);
        this->down_proj = new W4A8PerChnLinear<T>(intermediate_size, hidden_size);
    ...
    void prefill(int32_t num_tokens, T* input, void* output=nullptr) {
        ...
        this->down_proj->prefill(num_tokens, this->down_quant_invoker->output, this->down_quant_invoker->output_scale, this->down_quant_invoker->output_sum);
