#pragma once
#include "../model.cuh"
#include "w4a8_per_group_layer.cuh"


template <typename T>
struct W4A8PerGroupModelImpl: Model {
    Memory* memory;
    ...
    std::vector<W4A8PerGroupLayer<T>*> layers;
    RMSNorm<T>* norm;
    Linear<T>* lm_head;

    W4A8PerGroupModelImpl(
        ...
        for (int i = 0; i < num_hidden_layers; i++) {
            layers.push_back(new W4A8PerGroupLayer<T>(hidden_size, intermediate_size, num_attention_heads, num_key_value_heads, head_dim, rms_norm_eps));
        }
