...
#include <cublas_v2.h>
#include <cuda_fp16.h>
#include "../../qgemm/w4a8_per_group/w4a8_per_group_gemm_cuda.cuh"

template <typename T, bool transposed=true, bool has_bias=false>
struct W4A8PerGroupLinear{
    int dim_in;
    int dim_out;
    T* output;
    int8_t* weight;
    ...
    W4A8PerGroupLinear(int dim_in, int dim_out) {
        this->dim_in = dim_in;
    ...
    void prefill(const Stream& stream, int32_t num_tokens, int8_t* input, half* input_scale, T* tgt=nullptr) {
        ...
        w4a8_per_group_gemm_forward_cuda(
            stream,
            input,
            weight,
            s2_zeros,
