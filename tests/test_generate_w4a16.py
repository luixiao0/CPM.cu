...
# path = "/home/ydzhang/checkpoints/AutoGPTQ/Meta-Llama-3-8B-Instruct-4bit-128g-pileval-mse_shrink0.4"
...
if model_type == "medusa":
    llm = W4A16LLM_with_medusa(medusa_path, path, dtype=dtype, memory_limit=0.8, medusa_num_heads=3, medusa_choices=eval('mc_sim_7b_63_3head'), cuda_graph=cuda_graph, group_size=128, use_marlin=True)
    our_generate = lambda: llm.generate(input_ids, num_generate, teminators=teminators)
elif model_type == "eagle":
    llm = W4A16LLM_with_eagle(eagle_path, path, dtype=dtype, memory_limit=0.8, num_iter=6, tree_size=60, cuda_graph=cuda_graph, group_size=128, use_marlin=True)
    our_generate = lambda: llm.generate(input_ids, num_generate, teminators=[])
else:
    llm = W4A16LLM(path, dtype=dtype, memory_limit=0.8, cuda_graph=cuda_graph, group_size=128, use_marlin=True)
    our_generate = lambda: llm.generate(input_ids, num_generate, teminators=[])
