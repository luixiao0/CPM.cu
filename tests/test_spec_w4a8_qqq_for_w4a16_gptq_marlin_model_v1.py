...
from llamacu.speculative.spec_quant.spec_w4a8_qqq_for_w4a16_gm_model import W4A8QQQSpecW4A16GM
...
if model_type == "spec":
    ...
    llm = W4A8QQQSpecW4A16GM(drafter_path=draft_path, base_path=path, draft_num=6, dtype=dtype, chunk_length=chunk_length, memory_limit=0.8, cuda_graph=cuda_graph , draft_cuda_graph= draft_cuda_graph)
else:
    llm = W4A16GPTQMarlinLLM(path, dtype=dtype, memory_limit=0.5)
