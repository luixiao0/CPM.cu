...
from llamacu.speculative.spec_quant.spec_w4a16_gm_for_w4a16_gm_model_v0 import W4A16GMSpecW4A16GMV0
...
# input_ids = torch.cat([input_ids, torch.tensor([[8586]]).cuda().int()], dim=1)
...
if model_type == "spec":
    ...
    llm = W4A16GMSpecW4A16GMV0(drafter_path=draft_path, base_path=path, draft_num=6, dtype=dtype, chunk_length=chunk_length, memory_limit=0.8, cuda_graph=cuda_graph , draft_cuda_graph= draft_cuda_graph)
else:
