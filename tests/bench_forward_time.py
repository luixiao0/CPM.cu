...
output_file = "speed_output/Llama-3-8B-Instruct_llamacu.csv"
dtype = torch.float16
Bench = True
cuda_graph = True
num_tokens = 1024
...
print(our_logits)
print(f"our_logits shape: {our_logits.shape}")

# decode
verify_num_list = [1, 2, 4, 8, 16, 32, 64]
for num_verify in verify_num_list:
    ...
    with open(output_file, "a", newline='') as fout:
        fieldnames = ["prefix_len", "input_len", "forward_speed", "sec_per_forward"]
        ...
        ans_csv = {
            "prefix_len": 1024,
            "input_len": num_verify,
            "forward_speed": 1/our_time * 1000,
            "sec_per_forward": our_time,
        }
        writer.writerow(ans_csv)