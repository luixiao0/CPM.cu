...
with torch.no_grad():
    ...
    logits, past_key_values = model(input_ids, position_ids=position_ids, use_cache=True, return_dict=False)
    torch.cuda.nvtx.range_pop()
    if Bench:
        time = do_bench(lambda: model(input_ids, position_ids=position_ids, use_cache=True, return_dict=False), warmup=10, rep=1000)
...
torch.cuda.nvtx.range_pop()
if Bench:
    our_time = do_bench(lambda: llm.prefill(input_ids, position_ids), warmup=10, rep=1000)
...
with torch.no_grad():
    ...
    logits = model(input_ids, position_ids=position_ids, use_cache=True, past_key_values=past_key_values, return_dict=False)[0]
    torch.cuda.nvtx.range_pop()
    if Bench:
        time = do_bench(lambda: model(input_ids, position_ids=position_ids, use_cache=True, past_key_values=past_key_values, return_dict=False), warmup=10, rep=1000)

torch.cuda.nvtx.range_push("our decode")
our_logits = llm.decode(input_ids, position_ids, cache_length)
torch.cuda.nvtx.range_pop()
if Bench:
    our_time = do_bench(lambda: llm.decode(input_ids, position_ids, cache_length), warmup=10, rep=1000)
