        ...
        for suffix in supported_suffix_1:
            ...
            for suffix in supported_suffix_2:
                ...
                if len(files) > 1:
                    print(files)
                    if self.use_marlin:
                        if path + "/autogptq_model_gptq_marlin.safetensors" in files:
                            file = path + "/autogptq_model_gptq_marlin.safetensors"
                        else:
                            raise ValueError(f"Marlin models not found in {path}")
                    else:
                        if path + "/model.safetensors" in files:
